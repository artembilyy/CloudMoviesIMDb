...
final class ImageLoadingManager: ImageLoadingManagerProtocol {
    ...
    private func getURL(resizeFactor: Int, url: String) -> URL? {
        ...
        var result = url.replacingOccurrences(of: "(.*UX)([0-9]*)(.*)", with: "$1\(imgWidthScaled)$3", options: .regularExpression)
        result = result.replacingOccurrences(of: "(.*UX[0-9]*_CR0,[0-9]*,)([0-9]*)(.*)", with: "$1\(containerWidthScaled)$3", options: .regularExpression)
        result = result.replacingOccurrences(of: "(.*UX[0-9]*_CR0,[0-9]*,[0-9]*,)([0-9]*)(.*)", with: "$1\(containerHeightScaled)$3", options: .regularExpression)
